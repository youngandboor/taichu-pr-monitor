...
默认可以直接把 Gitea PR 的 `user.login` 作为 WeLink `--receiver`。实际 W3 账号通常为 `y` 加工号，生产运行建议提供 JSON 映射表，并加 `--require-recipient-map`，避免把未映射的 Gitea 昵称误当作 W3 账号。
...
映射表的键是 Gitea `user.login`，值是 W3 账号。示例见 `recipients.example.json`：
...
```bash
python3 -m monitor --recipients monitor/recipients.json --require-recipient-map
```
